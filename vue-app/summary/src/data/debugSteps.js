...
# 3 分层缓存: KV精确→向量→LLM 串行回退(为了省钱提速)`,
  ...
  },
  {
    ...
    title: '置信度阈值路由（客服分流）',
    description: 'Anthropic 路由模式：分类器给每条消息判类别+置信度，低于阈值就转人工。拖动阈值看自动率/人工率的权衡——练习题1的落地。',
    steps: [
      {
        name: '1. 分类器给每条消息判"类别+置信度"',
        description: '置信度=分类器"我有多确定"的分数(0~1)。真实系统里由 embedding+小分类模型或 LLM 产出。',
        code: `messages = [
  ("我要退款！！！",        类别=退款,      置信度=0.96),
  ...
  ("？？？",               类别=一级支持,  置信度=0.22),  # 无信息
        ...
        highlightLines: [4, 5],
        variables: [
          { name: '高置信', value: '意图清晰的消息，分类器很确定' },
          { name: '低置信', value: '含糊/太短/无信息，分类器没把握' },
        ],
        output: '10 条消息，各带类别和置信度'
      },
      {
        name: '2. 划一条阈值线',
        description: '阈值=你划的线。置信度≥阈值才自动处理，否则转人工。这条线怎么定，看"分错的代价"。',
        code: `threshold = 0.7   # 一级客服常用 0.6~0.7
for msg, category, conf in messages:
    if conf >= threshold:  自动 → category 队列
    else:                  转人工（没把握）`,
        highlightLines: [1, 3, 4],
        variables: [
          { name: '阈值高(0.9)', value: '准但转人工多(贵、慢)' },
          { name: '阈值低(0.6)', value: '省人工但错的多' },
        ],
        output: '每条消息按置信度分流：自动 or 人工'
      },
      {
        name: '3. 拖动阈值看权衡',
        description: '同一批消息，阈值不同，自动率天差地别。这就是练习题1要体会的：没有万能数字。',
        code: `阈值 0.30 → 9条自动(90%) 1条人工   # 几乎全自动，含糊的也硬分
阈值 0.70 → 6条自动(60%) 4条人工   # 平衡
阈值 0.95 → 1条自动(10%) 9条人工   # 只放行最确定的`,
        highlightLines: [1, 2, 3],
        variables: [
          { name: '一级客服', value: '分错代价低→阈值可低(0.6~0.7)' },
          { name: '退款/账户', value: '分错代价高→阈值调高(0.9)' },
        ],
        output: '阈值↑ 转人工↑(准但贵)；阈值↓ 自动↑(省但险)'
      },
      {
        name: '4. 生产级分类是分层的',
        description: 'LLM 会"自信地错"(高置信但答错)，所以路由主力不是 LLM，而是便宜可靠的小模型 + 分层兜底。',
        code: `消息进来：
  规则匹配命中?    → 命中就走(最便宜)
  小分类模型      → 置信度够高就走(主力，快)
  ...
  仍低于阈值      → 升级人工`,
        highlightLines: [2, 3, 4, 5],
        variables: [
          { name: '为啥不都用LLM', value: '路由高频调用，大LLM太贵太慢' },
          { name: '置信度谁更可信', value: 'embedding+小模型 > LLM自报' },
        ],
        output: '✅ 规则→小模型→LLM→人工，分层组合；置信度阈值是最后一道分流闸'
      },
    ]
  }
]
