...
#   聊天重时效 / 合规重重要性 / 检索重相关性` },
    ...
    },
    {
      id: 8,
      label: 'Day 8',
      date: '2026年7月1日 · Agent 工程 · Anthropic 五种工作流模式',
      footer: 'Day 8 · 2026-07-01 · Phase 14-12',
      progress: {
        label: '当前进度',
        detail: 'Phase 11 已全部学完 ✅ · Phase 14 · 已学 12 课',
        percent: 71,
        text: 'Phase 14 · 12 Anthropic 工作流模式',
        desc: '把前面零散方法收拢成 5 种标准工作流模式 + 选型图；重点做了路由的置信度阈值实验'
      },
      sections: [
        {
          emoji: '🗺',
          title: '—— 第 12 课：Anthropic 五种工作流模式 ——',
          blocks: [
            { type: 'text', text: '<strong>前面学的 ReAct/ReWOO/反思/搜索/CRITIC 都是零件，这一课把它们收拢成 5 种标准工作流模式。</strong>核心区分：<span class="highlight">工作流</span>(步骤可预先列举、图结构固定) vs <span class="highlight">Agent</span>(下一步取决于上一步结果、步数未知)。能列举步骤就用工作流。', style: 'note' }
          ]
        },
        {
          emoji: '🧩',
          ...
          tag: 'Phase 14-12',
          blocks: [
            { type: 'table', headers: ['模式', '一句话', '什么时候用'], rows: [
              ['提示词链', '上一步输出=下一步输入', '任务能线性分解'],
              ['路由', '分类器选走哪条下游', '不同输入要不同处理(客服分流)'],
              ['并行化', 'N 个调用并发跑再汇总', '分段处理 或 投票取多数'],
              ['编排器-工作者', '编排器动态派活给工作者', '子任务数量/类型运行时才定'],
              ['评估器-优化器', '一个提答案一个评分，循环到过', 'Self-Refine 的泛化(第5课)']
            ]},
            { type: 'text', text: '工作流 vs Agent：可预测/成本受限/合规受限 → 工作流(步数有限、图可审计)；开放研究/长度可变/新领域 → Agent(先探索后固化)。', style: 'note' }
          ]
        },
        {
          emoji: '🎚',
          title: '2. 重点：路由的置信度阈值（练习题1，已做成实验）',
          tag: 'Phase 14-12',
          blocks: [
            { type: 'list', items: [
              '<strong>置信度</strong>：分类器「我有多确定」的分数(0~1)；<strong>阈值</strong>：你划的线，低于线＝没把握→转人工',
              '<strong>权衡</strong>：阈值越高→转人工越多(准但贵慢)；越低→自动越多(省但错的多)。按「分错的代价」定',
              '一级客服分错代价低→阈值可低(0.6~0.7)；退款/账户安全分错代价高→调高(0.9)',
              '坑：LLM 会「自信地错」(高置信但答错)，置信度更信 embedding+小分类模型，而非 LLM 自报'
            ]},
            { type: 'table', headers: ['阈值', '自动处理', '转人工'], rows: [
              ['0.30', '90%（几乎全自动，含糊消息也硬分）', '10%'],
              ['0.70', '60%', '40%'],
              ['0.95', '10%（只放行最确定的）', '90%']
            ]},
            { type: 'text', text: '沙箱实验「置信度阈值路由」可拖阈值实时看这三行怎么变。生产级分类分层：规则匹配(最便宜)→小分类模型(主力)→LLM(兜底)→低于阈值升人工。', style: 'note' }
          ]
        },
        {
          emoji: '💬',
          ...
          blocks: [
            { type: 'qa', items: [
              { q: 'Anthropic 五种工作流模式是哪五个？', a: '提示词链(线性)、路由(分类分发)、并行化(并发+汇总)、编排器-工作者(动态派活)、评估器-优化器(提答案+评分循环)。都建立在「增强型 LLM」(带搜索/工具/记忆)之上。' },
              { q: '工作流和 Agent 怎么选？', a: '能列举步骤、成本/合规受限 → 工作流(步数有限、图可审计)；下一步取决于上一步结果、长度可变、新领域 → Agent。原则：能预测就用工作流，别为省事上 Agent。' },
              { q: '路由模式里置信度阈值起什么作用？设多高？', a: '分类器没把握(置信度低于阈值)就升级人工，而不是硬分一个类别。阈值按分错代价定：一级客服代价低可设 0.6~0.7，退款/账户安全代价高设 0.9。高阈值准但转人工多，低阈值省人工但错的多。' },
              { q: '生产里路由分类常用什么方式？为什么不都用 LLM？', a: '主力是 embedding+小分类模型(快、便宜、置信度可信)，配规则前置过滤 + LLM 兜底 + 人工升级。路由是高频调用，纯用大 LLM 太贵太慢，且 LLM 的自报置信度不如专用模型可靠。' }
            ]}
          ]
        },
        {
          emoji: '📌',
          title: 'Day 8 总结（Phase 14 · 12）',
          accentBorder: true,
          blocks: [
            { type: 'subtitle', text: '一张选型图收束前面所有零件' },
            { type: 'list', items: [
              '5 种工作流：提示词链 / 路由 / 并行化 / 编排器-工作者 / 评估器-优化器',
              '工作流 vs Agent：能列举步骤就用工作流(可预测、可审计、成本有界)',
              '路由的置信度阈值：没把握就转人工，阈值按「分错代价」定——本课重点实验'
            ]}
          ]
        }
      ]
