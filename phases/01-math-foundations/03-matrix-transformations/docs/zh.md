...
- 解释特征值为何决定了 PCA 方向、RNN 稳定性和谱聚类行为

## 问题

你读到 PCA，看到"求协方差矩阵的特征向量"。你读到模型稳定性，看到"检查所有特征值的模是否小于 1"。你读到数据增强，看到"应用随机旋转"。在你理解矩阵对空间做了什么几何操作之前，这些都没有意义。

矩阵不仅仅是数字网格。它们是空间机器。旋转矩阵旋转点。缩放矩阵拉伸它们。剪切矩阵倾斜它们。神经网络应用于数据的每一个变换都是这些操作之一或它们的组合。本课程让这些操作变得具体。

## 概念

### 作为矩阵的变换

二维中的每个线性变换都可以写成一个 2x2 矩阵。该矩阵精确告诉你基向量 [1, 0] 和 [0, 1] 最终去了哪里。其他一切由此而来。
...
先应用变换 A 再应用 B，等同于相乘它们的矩阵：`result = B @ A @ point`。顺序很重要。先旋转后缩放与先缩放后旋转产生不同的结果。
...
大多数向量在被矩阵作用时会改变方向。特征向量是特殊的：矩阵只缩放它们，从不旋转它们。缩放因子是特征值。
...
该矩阵沿 [1, 1] 方向将空间拉伸 3 倍，沿 [1, -1] 方向保持不变。每个其他方向都是这两个方向的混合。
...
**稳定性。** 在循环网络和动力系统中，模大于 1 的特征值会导致输出爆炸。模小于 1 会导致它们消失。这就是梯度消失/爆炸问题的一句话总结。
...
对于 2x2 矩阵 `[[a, b], [c, d]]`，特征值通过解特征方程求得：`lambda^2 - (a+d)*lambda + (ad - bc) = 0`。
...
本课程为 PCA（阶段 2）和神经网络权重分析奠定了几何基础。这里构建的特征值/特征向量代码与生产级 ML 系统中用于降维、谱聚类和稳定性分析的算法相同。

## 练习

1. 对单位正方形（顶点为 [0,0]、[1,0]、[1,1]、[0,1]）应用旋转、缩放和剪切。打印每个变换后的顶点坐标。验证旋转保持顶点之间的距离不变。

2. 手工使用特征方程求矩阵 [[4, 2], [1, 3]] 的特征值。然后用你从零实现的函数和 NumPy 验证。

3. 创建三个变换的组合（旋转 30 度、缩放 [1.5, 0.8]、剪切 kx=0.3），并将其应用于排列成圆形的 8 个点。打印变换前后的坐标。计算组合矩阵的行列式，验证它等于各个行列式的乘积。

## 关键术语

| 术语 | 人们怎么说 | 实际含义 |
|------|----------------|----------------------|
| 旋转矩阵 | "旋转东西" | 一种正交矩阵，沿圆弧移动点，保持距离和角度不变。行列式始终为 1。 |
| 缩放矩阵 | "放大东西" | 一种对角矩阵，沿每个轴独立拉伸或压缩。行列式是缩放因子的乘积。 |
| 剪切矩阵 | "倾斜东西" | 一种将一个坐标按比例移动到另一个坐标的矩阵，将矩形变成平行四边形。行列式为 1。 |
| 反射 | "镜像东西" | 一种跨轴或平面翻转空间的矩阵。行列式为 -1。 |
| 组合 | "做两件事" | 相乘变换矩阵以链式操作。顺序重要：B @ A 表示先应用 A，再应用 B。 |
| 特征向量 | "特殊方向" | 矩阵只缩放而不旋转的方向。变换的"指纹"。 |
| 特征值 | "拉伸多少" | 矩阵缩放其特征向量的标量因子。可以为负（翻转）或复数（旋转）。 |
| 特征分解 | "分解矩阵" | 将矩阵写为 V @ D @ V^(-1)，将其分解为其基本的缩放方向和大小。 |
| 行列式 | "来自矩阵的一个数字" | 变换缩放面积（2D）或体积（3D）的因子。零意味着变换不可逆。 |
