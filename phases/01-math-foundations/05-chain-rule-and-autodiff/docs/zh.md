...
- 仅使用从零实现的自动微分引擎构建并训练一个在 XOR 上的多层感知机
- 通过数值有限差分梯度检查验证自动微分的正确性

## 问题

你可以计算简单函数的导数。但神经网络不是一个简单函数。它是数百个函数复合在一起：矩阵乘法、加偏置、应用激活、再次矩阵乘法、softmax、交叉熵损失。输出是一个函数的函数的函数。

要训练网络，你需要损失关于每个单独权重的梯度。手工对数百万个参数做这件事是不可能的。用数值方法（有限差分）做又太慢。

链式法则给你数学工具。自动微分给你算法。它们结合在一起让你能够通过任意函数组合，在与一次前向传播成正比的时间内计算出精确梯度。
...
计算图使链式法则可视化。每个操作变成一个节点。数据沿计算图向前流动。梯度向后流动。
...
该图是动态的（按运行定义）。每次前向传播都会构建一个新图。这就是为什么 PyTorch 支持模型内的控制流（if/else、循环）。
...
基本的 Value 类处理加法、乘法和 relu。真正的自动微分引擎需要更多。以下是构建神经网络所需的运算：
...
巧妙之处：`__sub__` 和 `__truediv__` 是基于已有运算定义的。它们免费获得正确的梯度，因为链式法则通过底层的 add/mul/pow 运算进行了组合。

### 步骤 5：从零构建微型 MLP

有了完整的 Value 类，你可以构建一个神经网络。无需 PyTorch。无需 NumPy。只有 Value 和链式法则。
...
一个 `Neuron` 计算 `tanh(w1*x1 + w2*x2 + ... + b)`。一个 `Layer` 是神经元的列表。一个 `MLP` 堆叠多个层。每个权重都是一个 `Value`，所以调用 `loss.backward()` 将梯度传播到每个参数。
...
xs = [[0, 0], [0, 1], [1, 0], [1, 1]]
ys = [-1, 1, 1, -1]  # XOR 模式（使用 -1/1 以便 tanh）
...
这就是 micrograd。一个纯 Python 的完整神经网络训练循环，具有自动微分功能。每个商业深度学习框架都以更大规模做着同样的事。
...
梯度检查在实现新运算时至关重要。如果你的反向传播有 bug，数值检查会抓住它。每个严肃的深度学习实现在开发过程中都会进行梯度检查。
...
相同的梯度。你的引擎计算的结果与 PyTorch 相同，因为数学是一样的：通过链式法则的反向模式自动微分。
...
1. 为 Value 类添加 `__pow__`，使你可以计算 `x ** n`。验证 `d/dx(x^3)` 在 `x=2` 处等于 `12.0`。

2. 添加 `tanh` 作为激活函数。验证 `tanh'(0) = 1` 和 `tanh'(2) = 0.0707`（约值）。

3. 为单个神经元构建计算图：`y = relu(w1*x1 + w2*x2 + b)`。计算全部五个梯度并与 PyTorch 对照验证。

4. 使用对偶数实现前向模式自动微分。创建一个 `Dual` 类，验证它给出与反向模式引擎相同的导数。

## 关键术语

| 术语 | 人们怎么说 | 实际含义 |
|------|----------------|----------------------|
| 链式法则 | "乘导数" | 复合函数的导数等于每个函数局部导数的乘积，在相应点求值 |
| 计算图 | "网络图" | 一个有向无环图，节点是运算，边携带值（前向）或梯度（反向） |
| 前向模式 | "向前推导数" | 从输入到输出传播导数的自动微分。每个输入变量一次传播。 |
...
| 拓扑排序 | "依赖顺序" | 对图节点排序，使每个节点在其所有依赖之后出现。正确的梯度传播所需。 |
| 梯度累积 | "加，不替换" | 当一个值馈入多个运算时，其梯度是所有传入梯度贡献的总和 |
| 动态图 | "按运行定义" | 每次前向传播时重建的计算图，允许模型内部使用 Python 控制流（PyTorch 风格） |
...
- [PyTorch Autograd 机制](https://pytorch.org/docs/stable/notes/autograd.html) —— 真实系统的工作方式
- [Baydin 等，机器学习中的自动微分：综述](https://arxiv.org/abs/1502.05767) —— 综合参考
