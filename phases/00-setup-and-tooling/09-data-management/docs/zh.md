...
每个 AI 项目都从数据开始。你需要寻找数据集、下载它们、在格式之间转换、将它们划分为训练和评估集，并对它们进行版本管理，以便实验可以复现。每次都手动做这些工作既慢又容易出错。你需要一个可重复的工作流程。
...
graph TD
    ...
    E --> F["数据划分<br/>训练 / 验证 / 测试"]
    F --> G["你的训练流程"]
```

Hugging Face `datasets` 库是加载 AI 工作数据的标准方式。它开箱即用地处理下载、缓存、格式转换和流式传输。
...
这会下载 IMDB 电影评论数据集。第一次下载后，它会从 `~/.cache/huggingface/datasets/` 的缓存中加载。

### 步骤 3：流式传输大型数据集

有些数据集太大，无法放入磁盘。流式传输逐行加载数据，无需下载完整内容。
...
流式传输会返回一个 `IterableDataset`。你按行处理数据。无论数据集大小如何，内存使用量保持不变。
...
对于 AI 工作，Parquet 是最好的存储格式。Arrow 是你在内存中处理的格式。CSV 和 JSON 用于数据交换。
...
- **验证集（Validation）**：你在训练过程中检查进展（通常 10%）
- **测试集（Test）**：训练完成后的最终评估（通常 10%）

有些数据集自带预划分。如果没有，你可以自己划分：
...
DVC 创建小的 `.dvc` 文件，指向你的数据。数据本身存储在 S3、GCS 或其他远程存储后端。

| 方法 | 复杂度 | 最适合 |
|------|--------|--------|
| .gitignore | 低 | 个人项目、可以重新获取的已下载数据 |
| Git LFS | 中 | 通过 git 共享模型权重的团队 |
| DVC | 高 | 可复现的实验、大型数据集、团队 |

对于本课程，`.gitignore` 就足够了。当你需要在不同机器上复现精确实验时，使用 DVC。

### 步骤 8：存储模式

**本地存储**适用于约 10 GB 以下的数据集。HF 缓存会自动处理。

**云存储**用于更大的数据或跨机器共享的数据：
...
对于本课程，本地存储就足够了。当你在远程 GPU 实例上进行微调时，云存储变得相关。
...
1. 加载 `glue` 数据集（`mrpc` 配置）并检查前 5 个样本
2. 流式传输 `c4` 数据集，统计 10 秒内可以处理多少个样本
3. 将数据集转换为 Parquet 格式，并与 CSV 比较文件大小
...
| 术语 | 大家说的 | 实际含义 |
|------|---------|---------|
| 数据集划分（Dataset split） | "训练数据" | 在机器学习生命周期不同阶段使用的命名子集（训练/验证/测试） |
| 流式传输（Streaming） | "惰性加载" | 从远程源逐行处理数据，无需下载整个数据集 |
