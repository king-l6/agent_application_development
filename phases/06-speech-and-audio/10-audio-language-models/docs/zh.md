...
> 2026 年的音频语言模型能推理语音 + 环境声音 + 音乐。Qwen2.5-Omni-7B 在 MMAU-Pro 上与 GPT-4o Audio 持平。Audio Flamingo Next 在 LongAudioBench 上超越 Gemini 2.5 Pro。开源与闭源之间的差距基本消失 -- 除多音频任务外，所有人都在随机水平附近。

**类型：** 学习
**语言：** Python
**前置知识：** 阶段 6 · 04（ASR），阶段 12 · 03（视觉语言模型），阶段 7 · 10（音频 Transformer）
**时间：** ~45 分钟

## 问题

你有 5 秒音频：狗吠声，有人喊"停下！"，然后安静。有用的问题涵盖多个维度：

- **转录。** "说了什么？" -- ASR 的领域。
- **语义推理。** "这个人有危险吗？" -- 需要联合理解狗吠 + 喊声 + 安静。
- **音乐推理。** "哪些乐器在演奏旋律？"
- **长音频检索。** "在这 90 分钟的讲座中，讲师在哪里解释了梯度下降？"

一个能用一个提示回答所有这些问题的单一模型就是**音频语言模型**（LALM / ALM）。与纯 ASR 不同：LALM 生成自由形式的自然语言答案，而不仅仅是转录文本。
...
2. **投影器。** 线性层或 MLP，将音频编码器特征桥接到 LLM 的词元嵌入空间。
3. **LLM。** 基于 Llama / Qwen / Gemma 的解码器。接受交错的文本 + 音频词元；生成文本。

训练：

- **阶段 1。** 冻结编码器 + LLM；仅在 ASR / 字幕数据上训练投影器。
- **阶段 2。** 在指令跟随音频任务（QA、推理、音乐理解）上进行全量 / LoRA 微调。
...
**多音频列对所有人来说都很糟糕。** 4 选 1 多项选择的随机概率 = 25%；大多数模型的得分在附近。LALM 仍然难以比较两个片段。

### 2026 年 LALM 的实用场景

- **呼叫中心录音的合规审计。** "座席是否提到了必需的披露？"
- **无障碍。** 向听障用户描述声音事件（不仅仅是转录）。
- **内容审核。** 检测暴力语言 + 威胁性语气 + 背景上下文。
- **播客 / 会议分章。** 语义摘要，不仅仅是说话人切换。
- **音乐目录分析。** "找出所有包含 B 段转调的曲目。"

### 它们（尚）不适用的场景

- 精细音乐理论（低于和弦级别）。
- 对长对话的说话人归属推理（超过 10 分钟会退化）。
- 多音频比较（22-26% 仅略高于随机）。
- 实时流式推理（大多数为离线批处理推理）。
...
就是这样。投影器通常只有 1-3 个线性层。在 ASR 对（音频 → 转录）上训练它是阶段 1 的前置任务。
...
按类别（语音 / 声音 / 音乐 / 多音频）分别报告。汇总数字掩盖了模型失败的地方。
...
| 呼叫中心审计 | 通过 API 使用 Gemini 2.5 Pro，结合你的政策文档进行 RAG |

## 陷阱

- **对多音频过度信任。** 如果你的任务需要"哪个片段有 X"，随机水平的性能是真实存在的。
- **长音频退化。** 超过 10 分钟，大多数模型的说话人归属就会失效。先进行说话人日志（第 6 课），然后总结。
- **在静音上产生幻觉。** 同样的 Whisper 风格问题被使用 Whisper 编码器的 LALM 继承。使用 VAD 门控。
- **基准测试选择性引用。** 供应商博客文章突出显示最佳类别的结果。自己运行 MMAU-Pro 多音频子集。
...
1. **简单。** 运行 `code/main.py` 查看一个玩具投影器模式 + 假的 LALM 路由（音频嵌入、文本词元）→ 输出词元。
2. **中等。** 在 100 个 MMAU-Pro 语音项目上评测 Qwen2.5-Omni-7B。与论文报告的数字进行比较。
3. **困难。** 构建一个最小的音频字幕基线：BEATs 编码器 + 2 层投影器 + 冻结的 Llama-3.2-1B。仅在 AudioCaps 上微调投影器。与 SALMONN 在 Clotho-AQA 上比较。
...
| 语音输入 / 语音输出 | 语音原生 | 模型接收语音并发出语音，无需文本中转。 |
